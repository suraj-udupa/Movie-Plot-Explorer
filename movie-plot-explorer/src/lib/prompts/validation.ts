...
The user has provided a plot description. You need to identify real-world movies, TV shows, soap operas, or web series that match or are highly similar to this plot description!

PLOT DESCRIPTION FROM USER:
"${plot}"

Find up to 5 real titles (movies or series) that match this description. 
Rank them exactly according to their public average rating (e.g., IMDb rating where 10 is best). The highest-rated title should be rank 1.

For each match, provide:
- title (if it's a series, you can mention it e.g. "Title (TV Series)")
- description (brief synopsis)
- cast (array of strings, e.g. main 3-4 actors)
- director (or main showrunner)
- producer
- rating (number out of 10)
- rank (integer)
- matchReasoning (why this matches the user's description)
